	...
	if (!peer) {
		...
		if (dev->flags & WGDEVICE_HAS_BIND_ADDR) {
			if (dev->bind_addr.addr.sa_family == AF_INET) {
				...
				if(!memcmp(&dev->bind_addr.addr6.sin6_addr, &in6addr_any, sizeof(struct in6_addr)))
					dev->bind_addr.addr.sa_family = AF_UNSPEC;
...
static int parse_device(const struct nlattr *attr, void *data)
{
	...
	case WGDEVICE_A_BIND_ADDR:
		...
		if (addr->sa_family == AF_INET && mnl_attr_get_payload_len(attr) == sizeof(struct sockaddr_in))
			memcpy(&device->bind_addr.addr4, addr, sizeof(struct sockaddr_in));
		else if (addr->sa_family == AF_INET6 && mnl_attr_get_payload_len(attr) == sizeof(struct sockaddr_in6))
			memcpy(&device->bind_addr.addr6, addr, sizeof(struct sockaddr_in6));
		else
			break;
		device->flags |= WGDEVICE_HAS_BIND_ADDR;
		break;
